...
class BookingUltraCommon
{
	...
	function get_all_sytem_pages()
	{
	    ...
	    {
	        $this->wp_all_pages[0] = __( "Select Page", 'booking-ultra-pro');
    ...
    function create_plugin_setting($type, $id, $label, $pairs, $help, $inline_help = '', $extra=null) {
        ...
        switch ($type) {
            ...
            case 'checkbox':
                ...
                print "<input name=\"$id\" type=\"checkbox\" id=\"$id\" value=\"$value\" ".checked( $value, 1,false )." />";
                break;
            case 'color':
			...
			case 'checkbox_list':
                ...
                foreach ($pairs as $role_key => $role) {
                    if($default_role == $role_key){
                        echo esc_attr($this->check_box(array('name' => $id.'[]', 'id' => $id, 'value' => $role_key,'checked'=>'checked','disabled'=>'disabled')).$role.'<br/>');            
                    }else{
                        $checked_value = '';
                        if(is_array($selected_roles) && in_array($role_key,$selected_roles)){
                            $checked_value = $role_key;
                        }
                        echo esc_attr($this->check_box(array('name' => $id.'[]', 'id' => $id, 'value' => $role_key),$checked_value).$role.'<br/>');            
                    }
        ...
        if($inline_help!='')
        {
            echo '<a class="'.$qtip_classes.' uultra-tooltip" title="' . $inline_help . '" '.esc_attr($qtip_style).'><i class="fa fa-info-circle reg_tooltip"></i></a>';
        }
