...
class BookingUltraUserRegister {
		   ...
		   foreach($this->usermeta as $key => $value) 
			{
		        ...
		        if ($key == 'user_login') 
				{
		            if (esc_attr($value) == '') {
		                ...
		                $this->errors[] = __('<strong>ERROR:</strong> Please enter a username.','booking-ultra-pro');
						
		            } elseif (username_exists($value)) {
						
		               // $this->errors[] = __('<strong>ERROR:</strong> This username is already registered. Please choose another one.','booking-ultra-pro');
		            }
		        }
		        ...
		        if ($key == 'user_email') 
				{
		            if (esc_attr($value) == '') 
					{
		                $this->errors[] = __('<strong>ERROR:</strong> Please type your e-mail address.','booking-ultra-pro');
						
		            } elseif (!is_email($value)) 
					{
		                $this->errors[] = __('<strong>ERROR:</strong> The email address isn\'t correct.','booking-ultra-pro');
					
					} elseif ($value!=$_POST['user_email_2']) 
					{
		               // $this->errors[] = __('<strong>ERROR:</strong> The emails are different.','booking-ultra-pro');
	...
	function create_account_cart() 
	{
		...
		$custom_form =  esc_attr($_POST['bup-custom-form-id']);
		$filter_id =  esc_attr($_POST['bup-filter-id']);
		$template_id =  esc_attr($_POST['template_id']);		
		$full_number =  esc_attr($_POST['full_number']);
		$full_number_prefix =  esc_attr($_POST['full_number_prefix']);
		$full_number_iso =  esc_attr($_POST['full_number_iso']);
			...
			{			
				...
				if(email_exists($_POST['user_email']))
				{
					...
					$user_d = get_user_by( 'email', esc_attr($_POST['user_email'] ));
					$user_id  = $user_d->ID;
					...
					$user_id = wp_create_user( $sanitized_user_login, $user_pass, esc_attr($_POST['user_email']) );	
				...
				if (  $user_id ) 
				{
					...
					update_user_meta ($user_id, 'reg_telephone', esc_attr($_POST['telephone']));
					...
					update_user_meta($user_id, 'first_name',esc_attr($_POST['display_name']));
				...
				foreach ($CURRENT_CART as $key => $ITEM)  
				{
					...
					
					{
						...
						{						
							...
							{
								$value = esc_attr($_POST['telephone']);
							}					
				...
				{
					//payment Method
					$payment_method = esc_attr($_POST["bup_payment_method"]);	
					...
					//update status 					 
					  ...
					  elseif($_POST["bup_payment_method"]=='test'){  
					...
					if($payment_method=="paypal" && $amount > 0 && $payment_procesor)
					{
					 ...
					 }elseif($payment_method=="test" && !$payment_procesor){
	...
	function create_account() 
	{
		...
		$custom_form =  esc_attr($_POST['bup-custom-form-id']);
		$filter_id =  esc_attr($_POST['bup-filter-id']);
		$quantity =  esc_attr($_POST['bup-purchased-qty']);
		$template_id =  esc_attr($_POST['template_id']);
		$full_number = esc_attr( $_POST['full_number']);
		$full_number_prefix =  esc_attr($_POST['full_number_prefix']);
		$full_number_iso =  esc_attr($_POST['full_number_iso']);
	...
	//	exit;
		...
		if($quantity==''){$quantity=1;}
			...
			{				
				...
				$day_id = esc_attr($_POST['bup_date']);
				$staff_id = esc_attr($_POST['staff_id']);								
				$book_from = esc_attr($_POST['book_from']);
				$book_to = esc_attr($_POST['book_to']);
				...
				if(isset($bupcomplement))
				{
					...
					//mailchimp					 
					 ...
					 //$list_id = get_option( "buproaw_aweber_list");				 
					 if(isset($_POST["bup-sendinblue-confirmation"]) && $_POST["bup-sendinblue-confirmation"]==1 )				 {
						 
						$user_obj = get_user_by( 'id', $user_id );
						 $bupcomplement->sendinblue->create_contact( $user_obj );		
	...
	public function set_initial_booking_status($booking_id, $method)
	{
		...
		/*Update Appointment*/						
		$bookingultrapro->appointment->update_appointment_status($booking_id,$status,0);
