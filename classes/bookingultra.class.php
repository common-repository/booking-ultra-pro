...
class BookingUltraPro
{
	...
	public function hide_proversion_message () 
	{
		...
		$message= esc_attr($_POST['message_id']);		
	...
	<p class="submit">
		...
		<a href="?page=bookingultra&tab=licence" class="button-secondary" > '.esc_html__('Validate your Copy','booking-ultra-pro').'</a>
	</p>
</div>';
	...
	******************************************/
	...
	public function uultra_fresh_install_message ($message) 
	{
		...
		echo esc_attr("<p><strong>esc_attr($message)</strong></p></div>");
...
// Translators: %1$s is the placeholder for the service name, and %2$s is the placeholder for the provider's name.
$translatable_text = __('Below you can find a list of available time slots for %1$s by %2$s.', 'booking-ultra-pro');

$def_label = sprintf(
    $translatable_text,
    '<strong>[BUP_SERVICE]</strong>',
    '<strong>[BUP_PROVIDER]</strong>'
);
			...
			// Translators: %1$s is the service name, %2$s is the provider's name, %3$s is the time, and %4$s is the day.
$translatable_text = __('You\'re booking %1$s by %2$s at %3$s on %4$s', 'booking-ultra-pro');

$def_label = sprintf(
    $translatable_text,
    '<strong>[BUP_SERVICE]</strong>',
    '<strong>[BUP_PROVIDER]</strong>',
    '<strong>[BUP_AT]</strong>',
    '<strong>[BUP_DAY]</strong>'
);
	...
	{
		global $wp_locale,$bupcomplement;
		...
		$message_wait_submit ='<img src="'.BOOKINGUP_URL.'admin/images/loaderB16.gif" width="16" height="16" /></span>&nbsp; '.__("Please wait ...","booking-ultra-pro").'';	
		...
		// Translators: %s is a placeholder for the back navigation symbol.

		
		wp_localize_script( 'bup-front_js', 'bup_pro_front', array(
    'wait_submit'     => $message_wait_submit,
    'country_detection'     => $country_detection,
    'button_legend_step2'     => __('Search Again', "booking-ultra-pro"),

    // Translators: %s is the back navigation symbol.
    'button_legend_step3'     => sprintf(__('%s Back', 'booking-ultra-pro'), '<<'),

    'button_legend_step3_cart'     => __('Book More', "booking-ultra-pro"),
    ...
    // Translators: %s is the loading image and text.
    'message_wait_availability'     => '<p><img src="' . BOOKINGUP_URL . 'admin/images/loaderB16.gif" width="16" height="16" /></span>&nbsp; ' . __('Please wait ...', "booking-ultra-pro") . '</p>'
) );
		...
		$date_picker_array = array(
				...
				$days_array = array('dayslimit_key' => $days_limits, 'version' => $flag);
				wp_localize_script('bup-front_js', 'BUPdayslimitarray', $days_array);

		        /*custom days-limit-code-end*/
	...
	function display_registration_form_booking( $redirect_to=null , $args=null)
	{
		...
		{ 
			$custom_form=esc_attr($_GET["bup-custom-form-id"]);
		}
		...
		if($this->get_option('registration_rules')!=1)
		{
			...
			/*Test Payement*/ 	
			if($this->get_option('gateway_test_payment_active')=='1' && is_user_logged_in() && current_user_can('administrator'))
			{
				//custom label
				...
				if($custmom_label=='')
				{
					$custmom_label = __('Test Payment','booking-ultra-pro');
				
				}
				
				$display_payment_method = '<input type="radio" class="'.$required_class.' bup_payment_options" title="" name="bup_payment_method" id="bup_payment_method_test" value="test" data-method="test" /> <label for="bup_payment_method_test"><span></span>'.$custmom_label.'</label>';
													 
				$display .= '<div class="bup-profile-field">';
				$display .= '<label class="bup-field-type" for="bup_payment_method_test">';			
		...
		/*If sendinblue*/		
		if($this->get_option('newsletter_active')=='sendinblue' && $this->get_option('sendinblue_api')!="" && isset($bupcomplement))
		{
			
			//new sendinblue field			
			$sendinblue_text = stripslashes($this->get_option('sendinblue_text'));
			$sendinblue_header_text = stripslashes($this->get_option('sendinblue_header_text'));
			
			if($sendinblue_header_text==''){
				
				$sendinblue_header_text = __('Receive Daily Updates ', 'booking-ultra-pro');				
			}

			if($sendinblue_text==''){
				
				$sendinblue_text = __('Yes, I want to receive daily updates. ', 'booking-ultra-pro');
			}			
			
			
			//
			
			$sendinblue_autchecked = $this->get_option('sendinblue_auto_checked');
			
			$sendinblue_auto = '';
			if($sendinblue_autchecked==1){
				
				$sendinblue_auto = 'checked="checked"';				
			}
			
			 $display .= '<div class="bup-profile-separator">'.$sendinblue_header_text.'</div>';
			...
			//$display .= '<div class="bup-field-value">';
			 $display .= '<input type="checkbox"  title="'.$sendinblue_header_text.'" name="bup-sendinblue-confirmation"  id="bup-sendinblue-confirmation" value="1"  '.$sendinblue_auto.' > <label for="bup-sendinblue-confirmation"><span></span>'.$sendinblue_text.'</label>' ;
