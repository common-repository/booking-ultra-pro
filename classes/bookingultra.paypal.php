...
class BookingUltraPayment 
{
	...
	public function handle_paypal_ipn($paypal_response)
	{
		...
		$item_number = esc_attr($_POST['item_number']);
		$payment_status = esc_attr($_POST['payment_status']);
		$payment_amount = esc_attr($_POST['mc_gross']);
		$payment_currency = esc_attr($_POST['mc_currency']);
		$txn_id = esc_attr($_POST['txn_id']);
		$receiver_email = esc_attr($_POST['receiver_email']);
		$payer_email = esc_attr($_POST['payer_email']);
		$txn_type = esc_attr($_POST['txn_type']);
		$pending_reason = esc_attr($_POST['pending_reason']);
		$payment_type = esc_attr($_POST['payment_type']);
		$custom_key = esc_attr($_POST['custom']);
		
		//tweak for multi purchase
		$custom = explode("|", esc_attr($_POST['custom']));
	...
	{	
		...
		if($p_name==''){$p_name = $company_name.__(' - Purchase Details:', 'booking-ultra-pro');}
