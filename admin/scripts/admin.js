...
jQuery(document).ready(function($) {
	...
	function bup_set_auto_c()
	{
			...
			source: function( request, response ) {
				...
				open: function(event, ui) {
					...
					if(dialog.length > 0){
						$('.ui-autocomplete.ui-front').css('z-index', 10000);
					}
				},
	...
	jQuery(document).on("click", ".buppro-message-close", function(e) {
		...
		jQuery.ajax({
						type: 'POST',
						url: ajaxurl,
						data: {"action": "bup_hide_proversion_message","message_id": message_id ,_ajax_nonce: bup_admin_v98.nonce,
						 },
	...
	$(document).on("click", "#bup-backenedb-set-gacal-adm", function(e) {			
			...
			jQuery.ajax({
				type: 'POST',
				url: ajaxurl,
				data: {"action": "bup_set_default_gcal_staff", "google_calendar": google_calendar,  "staff_id": staff_id, _ajax_nonce: bup_admin_v98.nonce,},
	...
	jQuery(document).on("click touchstart", "#uultra-confirm-avatar-cropping", function(e) {
			...
			jQuery.ajax({
				type: 'POST',
				url: ajaxurl,
				data: {"action": "bup_crop_avatar_user_profile_image", "x1": x1 , "y1": y1 , "w": w , "h": h  , "image_id": image_id , "user_id": user_id, _ajax_nonce: bup_admin_v98.nonce,},
	...
	jQuery(document).on("click", "#bup-disconnect-gcal-user", function(e) {
    		...
    		jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: {"action": "bup_disconnect_user_gcal", "user_id": user_id, _ajax_nonce: bup_admin_v98.nonce, },
	...
	jQuery(document).on("click", "#btn-delete-user-avatar", function(e) {
    		...
    		jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: {"action": "bup_delete_user_avatar", "user_id": user_id, _ajax_nonce: bup_admin_v98.nonce, },
	...
	function refresh_my_avatar ()
		{
			 ...
			 jQuery.post(ajaxurl, {
							action: 'refresh_avatar', _ajax_nonce: bup_admin_v98.nonce, }, function (response){									
	...
	jQuery(document).on("click", "#bupadmin-btn-validate-copy", function(e) {	
		...
		jQuery.ajax({
						...
						"p_s_le": p_ded,
						_ajax_nonce: bup_admin_v98.nonce, },
						
						success: function(data){
							
							jQuery("#loading-animation").slideUp();							
						
								jQuery("#bup-vaalidation-results").html(data);
								jQuery("#bup-vaalidation-results").slideDown();								
								setTimeout("hidde_noti('bup-vaalidation-results')", 6000)
	...
	jQuery(document).on("click", "#bupadmin-btn-deactivate-copy", function(e) {	
		...
		jQuery.ajax({
						type: 'POST',
						url: ajaxurl,
						data: {"action": "bup_deactivate_license",
						_ajax_nonce: bup_admin_v98.nonce,},
	...
	jQuery(document).on("click", ".bup-btn-edit-field", function(e) {
		...
		jQuery.ajax({
						...
						"pos": block_id, "bup_custom_form": uultra_custom_form,_ajax_nonce: bup_admin_v98.nonce,},
	...
	jQuery(document).on("click", "#bup-adm-check-avail-btn", function(e) {
			...
			if(!(b_category>0))
			alert(err_message_service);
			jQuery("#bup-steps-cont-res" ).html( message_wait_availability );		
						
    		jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: {"action": "ubp_check_adm_availability", "b_category": b_category, "b_date": b_date , "b_staff": b_staff,_ajax_nonce: bup_admin_v98.nonce, },
		...
		jQuery(document).on("click", "#bup-adm-check-avail-btn-edit", function(e) {
    		...
    		jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: {"action": "ubp_check_adm_availability_admin", "b_category": b_category, "b_date": b_date , "b_staff": b_staff,_ajax_nonce: bup_admin_v98.nonce, },
	...
	jQuery(document).on("click", ".bup-btn-book-app-admin", function(e) {
			...
			jQuery.ajax({
					...
					data: {"action": "appointment_get_selected_time", 
						   "bup_booking_date": date_to_book,
						   "bup_service_staff": service_and_staff_id,
						   "bup_time_slot": time_slot,_ajax_nonce: bup_admin_v98.nonce,},
	...
	jQuery(document).on("change", "#bup-category", function(e) {
    		...
    		jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: {"action": "get_cate_dw_admin_ajax", "b_category": b_category,_ajax_nonce: bup_admin_v98.nonce,},
	...
	jQuery( "#bup-staff-editor-box" ).dialog({
			...
			autoOpen: false,			
			width: '900', // overcomes width:'auto' and maxWidth bug
   			maxWidth: 900,
			responsive: true,
			...
			"Add": function() {				
				...
				var bup_mon_from=   jQuery("#bup-mon-from-new").val();
				var bup_mon_to=   jQuery("#bup-mon-to-new").val();			
				var bup_tue_from=   jQuery("#bup-tue-from-new").val();
				var bup_tue_to=   jQuery("#bup-tue-to-new").val();			
				var bup_wed_from=   jQuery("#bup-wed-from-new").val();
				var bup_wed_to=   jQuery("#bup-wed-to-new").val();			
				var bup_thu_from=   jQuery("#bup-thu-from-new").val();
				var bup_thu_to=   jQuery("#bup-thu-to-new").val();			
				var bup_fri_from=   jQuery("#bup-fri-from-new").val();
				var bup_fri_to=   jQuery("#bup-fri-to-new").val();			
				var bup_sat_from=   jQuery("#bup-sat-from-new").val();
				var bup_sat_to=   jQuery("#bup-sat-to-new").val();			
				var bup_sun_from=   jQuery("#bup-sun-from-new").val();
				var bup_sun_to=   jQuery("#bup-sun-to-new").val();	

				if(bup_mon_from.length == 0 && bup_tue_from.length  == 0 && bup_wed_from.length  == 0 
					&& bup_thu_from.length  == 0  && bup_fri_from.length  == 0  && bup_sat_from.length  == 0  && bup_sun_from.length  == 0 )
				{
					alert('Select Any Schedule Interval');
				}
				else
				{			
				jQuery.ajax({
						type: 'POST',
						url: ajaxurl,
						data: {"action": "ubp_add_staff_confirm", "staff_name": staff_name, "staff_email": staff_email , "staff_nick": staff_nick ,_ajax_nonce: bup_admin_v98.nonce,},
						...
						success: function(data){
							...
							if(isInteger(res))	
							{
								...
								//update the services that new staff selects
								...
								jQuery.ajax({
										type: 'POST',
										url: ajaxurl,
										data: {"action": "ubp_update_staff_services",  "service_list": service_list,  "staff_id": staff_id ,_ajax_nonce: bup_admin_v98.nonce, },
								...
								// Updates the Schedule that new staff selects
	...
	jQuery( "#bup-client-new-box" ).dialog({
			...
			"Add": function() {				
				...
				jQuery.ajax({
						type: 'POST',
						url: ajaxurl,
						data: {"action": "ubp_add_client_confirm", "client_name": client_name, "client_last_name": client_last_name , "client_email": client_email, "client_phone": client_phone,_ajax_nonce : bup_admin_v98.nonce,  },
	...
	jQuery(document).on("click", ".bup-payment-deletion", function(e) {
		    ...
		    if(doIt)
		    {
				jQuery.ajax({
						type: 'POST',
						url: ajaxurl,
						data: {"action": "bup_delete_payment",  "payment_id": payment_id ,  "appointment_id": appointment_id, _ajax_nonce: bup_admin_v98.nonce,  },
	...
	jQuery(document).on("click", ".bup-note-deletion", function(e) {
		    ...
		    if(doIt)
		    {
				jQuery.ajax({
						type: 'POST',
						url: ajaxurl,
						data: {"action": "bup_delete_note",  "note_id": note_id ,  "appointment_id": appointment_id ,_ajax_nonce: bup_admin_v98.nonce,  },
	...
	jQuery(document).on("click", ".bup-payment-edit", function(e) {
			...
			jQuery.ajax({
						type: 'POST',
						url: ajaxurl,
						data: {"action": "bup_get_payment_form",  "payment_id": payment_id ,  "appointment_id": appointment_id, _ajax_nonce: bup_admin_v98.nonce,  },
	...
	jQuery(document).on("click", "#bup-adm-add-payment", function(e) {
    		...
    		jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: {"action": "bup_get_payment_form", _ajax_nonce: bup_admin_v98.nonce, },
	...
	jQuery(document).on("click", ".bup-breaks-add", function(e) {
    		...
    		jQuery.ajax({
					...
					data: {"action": "bup_get_break_add", 
							"day_id": day_id,
							"staff_id": staff_id , _ajax_nonce: bup_admin_v98.nonce, },
	...
	jQuery(document).on("click", "#bup-adm-add-note", function(e) {
    		...
    		jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: {"action": "bup_get_note_form", _ajax_nonce: bup_admin_v98.nonce, },
	...
	jQuery(document).on("click", "#bup-adm-update-info", function(e) {
    		...
    		jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: {"action": "bup_update_booking_info", "custom_fields": serial_data, "booking_id": booking_id, _ajax_nonce: bup_admin_v98.nonce, },
	...
	jQuery(document).on("click", "#bup-add-category-btn", function(e) {
    		...
    		jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: {"action": "bup_get_category_add_form" , _ajax_nonce: bup_admin_v98.nonce, },
	...
	jQuery(document).on("click", ".bup-edit-category-btn", function(e) {
    		...
    		jQuery.ajax({
					...
					data: {"action": "bup_get_category_add_form",
						"category_id": category_id, _ajax_nonce: bup_admin_v98.nonce, },
	...
	jQuery( "#bup-service-editor-box" ).dialog({
			...
			"Update": function() {				
				...
				var service_pending_color =  jQuery("#bup-service-pending-color" ).val();
	...
	jQuery( "#bup-service-add-box" ).dialog({
		autoOpen: false,																							
		width: 550,
		modal: true,
		buttons: {
		"Add": function() {				
			...
			jQuery.ajax({
						...
						success: function(data){	
							...
							jQuery("#bup-service-add-box" ).dialog( "close" );				
							bup_load_services();
	...
	jQuery(document).on("click", "#ubp-add-staff-btn", function(e) {
    		...
    		jQuery.ajax({
					...
					success: function(data){								
						...
						//jQuery('#tab-1').css('border', '1px solid #2271b1');
						jQuery('#tab-1').addClass('bup-staff-tab-active');
						//jQuery('#tab-1.nav-tab-active').css('background', '#2271b1');
						jQuery("#bup-spinner").hide();
	...
	// staff box scripts starts
		
	jQuery(document).on("click", "#showservices,#Service-tab", function(e) {

		var staff_name=   jQuery("#staff_name").val();
		var staff_email=   jQuery("#staff_email").val();
		var staff_nick=   jQuery("#staff_nick").val();

		if(staff_name.length == 0 || staff_nick.length == 0 || staff_email.length == 0)
		{
			jQuery('showservices').prop('disabled', true);
			alert('Please fill the Staff Details first');
		}
		else{
		jQuery("#show-tab-staffservices").show();
		jQuery(".ui-dialog-buttonpane").hide();
		jQuery("#show-tab-staffdetails").css("display", "none");
		jQuery("#show-tab-staffschedules").css("display", "none");
		// jQuery('#tab-1').css('border', '3px solid white');
		// jQuery('#tab-2').css('border', '1px solid #2271b1');
		// jQuery('#tab-3').css('border', '3px solid white');

		jQuery('#tab-1').removeClass('bup-staff-tab-active');
		jQuery('#tab-2').addClass('bup-staff-tab-active');		
		jQuery('#tab-3').removeClass('bup-staff-tab-active');
		}
	});



	jQuery(document).on("click", "#showschedules,#Schedule-tab", function(e) {

		var service_list = ubp_get_checked_services();

		if(service_list.length == 0){
			jQuery('showschedules').prop('disabled', true);
			alert('Please Select the Service');
		}
		else{
		jQuery("#show-tab-staffschedules").show();
		jQuery(".ui-dialog-buttonpane").show();
		jQuery("#show-tab-staffservices").css("display", "none");
		jQuery("#show-tab-staffdetails").css("display", "none");
		
		// jQuery('#tab-1').css('border', '3px solid white');
		// jQuery('#tab-2').css('border', '3px solid white');
		// jQuery('#tab-3').css('border', '1px solid #2271b1');

		jQuery('#tab-1').removeClass('bup-staff-tab-active');
		jQuery('#tab-2').removeClass('bup-staff-tab-active');		
		jQuery('#tab-3').addClass('bup-staff-tab-active');
		}
	});

	jQuery(document).on("click", "#showdetails", function(e) {
		jQuery("#show-tab-staffdetails").show();
		jQuery(".ui-dialog-buttonpane").hide();
		jQuery("#show-tab-staffservices").css("display", "none");
		jQuery("#show-tab-staffschedules").css("display", "none");
		jQuery('#tab-1').css('border', '1px solid #2271b1');
	...
	jQuery(document).on("click", "#Details-tab", function(e) {
		jQuery("#show-tab-staffdetails").show();
		jQuery(".ui-dialog-buttonpane").hide();
		jQuery("#show-tab-staffservices").css("display", "none");
		jQuery("#show-tab-staffschedules").css("display", "none");
	...
	jQuery(document).on("click", "#tab-1", function(e) {
		
		// jQuery('#tab-1').css('border', '1px solid #2271b1');
		// jQuery('#tab-2').css('border', '1px solid white');
		// jQuery('#tab-3').css('border', '1px solid white');

		jQuery('#tab-1').addClass('bup-staff-tab-active');
		jQuery('#tab-2').removeClass('bup-staff-tab-active');		
		jQuery('#tab-3').removeClass('bup-staff-tab-active');
	});
	jQuery(document).on("click", "#tab-2", function(e) {
	...
	jQuery(document).on("click", "#tab-3", function(e) {
	...
	jQuery(document).on("click", "#bup-add-service-btn", function(e) {
			...
			var cate_count = jQuery('#bup-categories-list ul li').length;
			if(cate_count == 0)
			{
			...
			{
			jQuery('#bup-service-add-box').dialog('option', 'title', bup_admin_v98.msg_service_add);			
			jQuery("#bup-spinner").show();
				
    		jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: {"action": "ubp_get_service",  "service_id": service_id ,  "cate_id": cate_id , _ajax_nonce: bup_admin_v98.nonce, },
					
					success: function(data){		
					
					
						jQuery("#bup-service-add-box" ).html( data );							
						jQuery("#bup-service-add-box" ).dialog( "open" );
						jQuery('.active-color-picker').wpColorPicker();
						jQuery('.pending-color-picker').wpColorPicker();
						
						jQuery('.font-color-picker').wpColorPicker();
						jQuery("#bup-spinner").hide();	
						jQuery("#help-text-tooltip").tooltip();
	...
	jQuery(document).on("click", ".bup-admin-edit-service", function(e) {
		...
		jQuery(document).on("click", "#bup-admin-edit-staff-location-save", function(e) {
    		...
    		jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: {"action": "ubp_update_staff_locations",  "location_list": location_list,  "staff_id": staff_id ,_ajax_nonce: bup_admin_v98.nonce, },
		  ...
		  if(doIt)
		  {
			...
			var uultra_custom_form = jQuery('#uultra__custom_registration_form').val();
				...
				jQuery.ajax({
						...
						"p_confirm": "yes"  , 		"bup_custom_form": uultra_custom_form ,_ajax_nonce: bup_admin_v98.nonce, },
	...
	jQuery( "#bup-service-variable-pricing-box" ).dialog({
			...
			"Save & Exit": function() {				
				...
				jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: {"action": "bup_update_group_pricing_table", "pricing_list": pricing_list , "service_id": service_id,_ajax_nonce: bup_admin_v98.nonce,  },
	...
	jQuery(document).on("click", ".bup-admin-edit-pricing", function(e) {
    		...
    		jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: {"action": "bup_get_service_pricing", "service_id": service_id, _ajax_nonce: bup_admin_v98.nonce,  },
	...
	jQuery(document).on("click", "#search_staff", function(e)	
	{
		e.preventDefault();
		jQuery("#bup-spinner").show();
		$search_staff_keyword=document.getElementById("search_staff_keyword").value;
		let curr_url=window.location.href;
		if(curr_url.search("paged=")>0)
		$paged = window.location.href.slice(window.location.href.indexOf('paged=')+6);// added to resolve the issue 
		else
		$paged =1;
		...
		jQuery.ajax({
						type: 'GET',
						url: ajaxurl,
						data: {"action": "bup_get_staff_list_admin_ajax","paged":$paged,"search_staff_keyword":$search_staff_keyword },
...
{					
    		...
    		jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: {"action": "bup_get_payments_list",  "appointment_id": appointment_id,_ajax_nonce: bup_admin_v98.nonce, },
...
{					
    		...
    		jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: {"action": "bup_get_notes_list",  "appointment_id": appointment_id, _ajax_nonce: bup_admin_v98.nonce, },
...
function bup_load_staff_under_category(appointment_id)	
{
	...
	var b_category=   jQuery("#bup-category").val();
    		...
    		jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: {"action": "get_cate_dw_admin_ajax", "b_category": b_category, "appointment_id": appointment_id,_ajax_nonce: bup_admin_v98.nonce, },
...
function bup_load_staff_list_adm()	
{
    ...
    jQuery.ajax({
					type: 'GET',
					url: ajaxurl,
					data: {"action": "bup_get_staff_list_admin_ajax","paged":$paged},
...
function bup_load_staff_details(staff_id)	
{
    ...
    jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: {"action": "bup_get_staff_details_admin", "staff_id": staff_id,_ajax_nonce: bup_admin_v98.nonce, },
...
function bup_edit_appointment_inline(appointment_id, conf_message, show_conf_message)	
{
	...
	jQuery.ajax({
				  ...
				  data: {"action": "bup_admin_edit_appointment", 
				         "appointment_id": appointment_id,_ajax_nonce: bup_admin_v98.nonce, },
